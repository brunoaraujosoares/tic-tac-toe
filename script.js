...
function checkForWin(){
    ...
    for (const line of lines) {
        const hasCross = line.every(
            ( square ) => square.classList.contains("cross") 
        );
        const hasCircle = line.every(
            ( square ) => square.classList.contains("circle")
        ) ; 
...
function playerWon(){
    infoText.innerHTML = `${pastPlayer} wins!`
    playerHasWon = true;
    continueGame();
}
...
function reset(){
    allSquares.forEach( (square) => { square.classList = "grid__square" } )
    addSquareClick();
    playerHasWon = false;
...
function startGame(){
    ...
    form.addEventListener('submit', (event) =>  {
        ...
        const player2Input = document.getElementById("player2").value.trim().toLowerCase();
         ...
         const player1Cap = player1Input.charAt(0).toUpperCase() + player1Input.slice(1) ;
         const player2Cap = player2Input.charAt(0).toUpperCase() + player2Input.slice(1) ;

         players.playerOne.name = player1Cap;
         players.playerTwo.name = player2Cap;

         nextPlayer = player1Cap;
         ...
         startGameBtn.innerHTML = "Reset Score";
         addSquareClick();
         restartGame();
    });
}
